...
class MaskConv2d(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
        ...
        self.register_buffer("d", torch.zeros(self.weight.data.size(1)))
        self.pruned_weight = Parameter(torch.zeros_like(self.weight,).cuda())
        # self.pruned_weight.data.fill_(0)

    def forward(self, input):
        return F.conv2d(input, self.pruned_weight.cuda(), self.bias, self.stride,
